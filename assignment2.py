...
def listallusers():
    if(request.method!='GET'):
        ...
        r1=requests.post("http://127.0.0.1:8000/api/v1/db/read",json=a)
        users=resp1.json()
        if(len(users)<=0):
            abort(204)
        else:
...
def userclear():
    if(request.method!="POST"):
        ...
        a={"table":"user","columns":["username"],"where":"username like '%'"}
        r1=requests.post("http://127.0.0.1:8000/api/v1/db/read",json=a)
        c={"table":"participant","columns":["*"],"where":"rideid like '%'"}
        r3=requests.delete("http://127.0.0.1:8080/api/v1/db/read",json=c)
        b={"table":"ride","columns":["*"],"where":"rideid like '%'"}
        r2=requests.delete("http://127.0.0.1:8080/api/v1/db/read",json=b)
